...
function renderCollection(arr, instant=false){
  ...
  const src = arr.filter(it => {
    ...
    if (activeFilter === 'stock') return !it.status || it.status !== 'preorder';
  ...
  src.forEach(it=>{
    ...
    if (!isCorrectSection) {
      ...
      // "в наличии" - товары без статуса или со статусом не "preorder"
      shouldShow = !it.status || it.status !== 'preorder';

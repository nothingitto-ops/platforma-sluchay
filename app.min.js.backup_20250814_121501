...
function renderProducts() {
  ...
  if ($catalog.children.length === 0) {
    items.forEach(item => {
      ...
      card.dataset.availability = item.status; // для фильтрации
  ...
  let visibleCount = 0;
  const cards = Array.from($catalog.children).filter(child => child.classList.contains('card'));
  
  // Функция для определения, должна ли карточка быть видимой
  const shouldShow = (card) => {
    ...
    // Проверяем фильтр
    if (activeFilter === 'all') return true;
    if (activeFilter === 'stock') return status === 'stock';
    if (activeFilter === 'preorder') return status === 'preorder';
    
    return false;
  };
  
  // Сначала скрываем все карточки, которые должны быть скрыты
  cards.forEach(card => {
    if (!shouldShow(card) && !card.classList.contains('is-hidden')) {
      card.classList.add('is-hiding');
    }
  });
  
  // Ждем завершения анимации скрытия, затем показываем нужные
  setTimeout(() => {
    cards.forEach(card => {
      if (!shouldShow(card)) {
...
function switchTab(tabName) {
  ...
  // Восстанавливаем фильтр для этой вкладки
  activeFilter = filterByTab[tabName] || 'all';
...
function switchFilter(filterName) {
  ...
  filterByTab[currentTab] = filterName; // сохраняем фильтр для текущей вкладки
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', (e) => {
      e.preventDefault(); // убираем прыжок
  ...
  // Убираем переходы для быстрого отображения
  if ($catalog) {

...
function renderCollection(arr, instant=false){
  ...
  // Добавляем плавные переходы только для первого рендера, не для переключения фильтров
  if(!instant && !$catalog.hasAttribute('data-initialized')){ 
    ...
    // Для мгновенного рендера (переключение фильтров) полностью убираем анимацию
...
function renderActive(instant=false){
  ...
  // Отмечаем, что каталог инициализирован и восстанавливаем анимацию
  if(!$catalog.hasAttribute('data-initialized')) {
    $catalog.setAttribute('data-initialized', 'true');
    // Восстанавливаем анимацию после первого рендера
    setTimeout(() => {
      $catalog.style.transition = 'opacity 0.3s ease-out, transform 0.3s ease-out';
    }, 100);
  }

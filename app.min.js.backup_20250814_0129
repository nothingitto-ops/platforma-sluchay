...
function renderCollection(arr, instant=false){
  ...
  const src = arr.filter(it => {
    ...
    // Проверяем, что товар принадлежит текущему разделу
    const isCorrectSection = (currentTab === 'home' && it.section === 'home') || 
                            (currentTab === 'nessffo' && it.section === 'nessffo');
    
    if (!isCorrectSection) return false;
    
    // Применяем фильтр по статусу
    if (activeFilter === 'all') return true;
    if (activeFilter === 'stock') return it.status === 'stock';
    if (activeFilter === 'preorder') return it.status === 'preorder';
    return false;
  }).sort((a, b) => (a.order || 999) - (b.order || 999));

  closeModal();
  
  // Добавляем плавные переходы только для первого рендера
  if(!instant && !$catalog.hasAttribute('data-initialized')){ 
    $catalog.style.transition = 'opacity 0.4s ease-out, transform 0.4s ease-out';
    $catalog.style.opacity='0'; 
    $catalog.style.visibility='hidden';
    $catalog.style.transform = 'translateY(10px)';
  } else {
    // Для всех остальных случаев (переключение фильтров, вкладок) убираем анимацию
  ...
  // Для мгновенного рендера используем существующие карточки
  if(instant && $catalog.children.length > 0) {
    // Скрываем все карточки и удаляем сообщение "пусто"
    Array.from($catalog.children).forEach(child => {
      if(child.classList.contains('card') || child.classList.contains('placeholder')) {
        child.style.display = 'none';
      } else if(child.classList.contains('empty')) {
        child.remove();
      }
    });
  } else {
    // Очищаем содержимое только для первого рендера
    $catalog.innerHTML='';
  }
  
  // Принудительно устанавливаем стили для мгновенного рендера
  if(instant) {
    $catalog.style.transition = 'none !important';
    $catalog.style.opacity = '1 !important';
    $catalog.style.visibility = 'visible !important';
    $catalog.style.transform = 'translateY(0) !important';
  }

  const realCount = arr.filter(it => {
    if (it.placeholder) return false;
  ...
  src.forEach(it=>{
    // Для мгновенного рендера ищем существующую карточку
    let card;
    if(instant) {
      card = Array.from($catalog.children).find(child => 
        child.querySelector('h3') && child.querySelector('h3').textContent === it.title
      );
    }
    
    if(!card) {
      // Создаем новую карточку только если не найдена существующая
      ...
      // Показываем существующую карточку и обновляем статус если нужно
      card.style.display = '';
      // Обновляем статус карточки если он изменился
      const statusEl = card.querySelector('.status');
      if(!it.placeholder && it.status) {
        const cls = (it.status==='preorder')?'pre':'in';
        const statusText = (it.status==='preorder')?'под заказ':'в наличии';
        if(statusEl) {
    ...
    // Проверяем, соответствует ли карточка текущему фильтру и разделу
    ...
    if (!isCorrectSection) {
      ...
      // "в наличии" - товары со статусом "stock"
      shouldShow = it.status === 'stock';
    } else if (activeFilter === 'preorder') {
      // "под заказ" - товары со статусом "preorder"
      shouldShow = it.status === 'preorder';
    }
    ...
    if(!it.placeholder){
      // Получаем изображение и imgBox из карточки
      const img = card.querySelector('img');
      const imgBox = card.querySelector('.card-img');
      if(img && imgBox) {
        img.dataset.key = it.images[0];
        let hoverRaf = 0;
        let currentHoverIndex = 0;
        let isHovering = false;
        
        // Мгновенно загружаем первое изображение
        img.src = it.images[0];
        
        // Предзагружаем остальные изображения в фоне для быстрого hover
        // Используем более агрессивную предзагрузку для медленного интернета
        setTimeout(() => {
          preloadImages(it.images.slice(1));
        }, 100);
        
        // Дополнительная предзагрузка через 500мс для медленного интернета
        setTimeout(() => {
          preloadImages(it.images.slice(1));
        }, 500);
        
        const applyIdx = (idx) => {
          if (idx < 0 || idx >= it.images.length) return;
          const key = it.images[idx];
          if (img.dataset.key !== key) {
            img.dataset.key = key;
            currentHoverIndex = idx;
            // Мгновенная смена изображения без ожидания загрузки
...
function renderActive(instant=false){
  if(!$catalog) return;
  // Передаем все товары, фильтрация будет в renderCollection
  const arr = items;
  ...
  // Отмечаем, что каталог инициализирован
  if(!$catalog.hasAttribute('data-initialized')) {
    $catalog.setAttribute('data-initialized', 'true');
  }
...
function preloadImages(imagePaths) {
  imagePaths.forEach(path => {
    if (path && !path.startsWith('data:')) {
      ...
      img.onerror = () => {
        // console.warn('Не удалось предзагрузить изображение:', path);
      };
      // Устанавливаем src в конце для лучшей совместимости
      img.src = path;
    }
  });
}
...
// Обновлено: Thu Aug 14 01:15:20 MSK 2025
